...
class ModuleTask extends AppShell {
    ...
    public function listModules($numerate = false, $type = 'module') {
        ...
        foreach ($modules as $module) {
            ...
            $this->out("{$prefix}{$yaml['name']}{$version}");
        }
    ...
    protected function _read() {
        $yaml = array();
        $moduleAlias = Inflector::camelize($this->in(__d('system', 'Alias name of the module, in CamelCase. e.g.: "MyTestModule"')));
        $yaml['name'] = $this->in(__d('system', 'Human readable name of the module. e.g.: "My Test Module"'));
        $yaml['description'] = $this->in(__d('system', 'Brief description'));
        $yaml['category'] = 'Core';

        while (Inflector::camelize($yaml['category']) == 'Core') {
            $yaml['category'] = $this->in(__d('system', 'Category'));
        ...
        $yaml['version'] = $this->in(__d('system', 'Module version. e.g.: 1.0, 2.0.1'));
        $yaml['core'] = $this->in(__d('system', 'Required version of Quickapps CMS. e.g: 1.x, >=1.0'));
    ...
    protected function _build($path, $info) {
        ...
        $source = dirname(dirname(dirname(__FILE__))) . DS . 'Templates' . DS . 'qa_module';
        $Folder = new Folder();
        ...
        if ($this->__rcopy($source, $path . $info['alias'])) {
            ...
            foreach ($files as $file) {
                ...
                if ($this->__file_ext($fileName) != 'yaml') {
                    $this->__replace_file_content($file, '/ModuleName/', $info['alias']);
                }
                ...
                if (strpos($fileName, "ModuleName") !== false) {
                    rename($file, str_replace("ModuleName", $info['alias'], $file));
                }
            }
